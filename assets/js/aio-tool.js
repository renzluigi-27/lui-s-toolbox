...
function updateTabUI() {
  ...
  if (rerouteCard) rerouteCard.style.display = activeMode === 'payout' ? 'block' : 'none';
  updateRefHint();
  updateGenerateBtn();
}
...
// ─────────────────────────────────────────────────────────────────
// FILE UPLOAD — updated payment info sheet (rerouted clients · payout mode)
// ─────────────────────────────────────────────────────────────────
...
const rerouteInputEl = document.getElementById('rerouteFileInput');
if (rerouteInputEl) {
  rerouteInputEl.addEventListener('change', e => {
...
function handleRerouteFile(file) {
  ...
  readExcel(file, rows => {
    ...
    const loaded = document.getElementById('rerouteFileLoaded');
...
// Build override map keyed by container + client name (handles shared containers)
...
// DEDUCTION SCHEDULE
// ─────────────────────────────────────────────────────────────────
// New size-based insurance — applies ONLY to new clients whose first
// payout is on/after 30 Jun 2026. Old / rerouted clients stay flat 1,500.
...
// Insurance / HC deduction basis date.
// New & never-paid containers count their yearly anniversaries (and size-based
// pricing) from the PAYOUT RESTART date, not the original first payout —
// because they were never actually paid on the original date, so the restart
// date IS their real first payout. Two cases use the restart date:
//   1. first payout == restart date  → totally new (e.g. both 30 Jun)
//   2. first payout on/after 1 Mar 2026 → never-paid batch (15 Mar / 31 Mar /
//      15 Apr / 30 Apr ... whose payout was rescheduled to the restart date)
// Old clients (first payout before 1 Mar 2026) keep their original first payout.
...
function runEmailMatcher(yr, mo, cycle, cycleOpt, nameMode) {
  ...
  // Cycle filter — all clients ignores cycle, by cycle filters by 15th or EOM
  ...
  // Name mode — unique deduped or repeating per payment sheet row
...
// ─────────────────────────────────────────────────────────────────
// EMAIL MATCHER OPTIONS — state, helpers, collapsible card
// ─────────────────────────────────────────────────────────────────
